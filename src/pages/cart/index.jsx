    ...
    const handleBuyNow = () => {
        const selectedProductsQueryParam = JSON.stringify(
            checkedProducts.map((productId) => {
                const selectedProduct = products.find((product) => product._id === productId);
                if (selectedProduct) {
                    const { quantity, productId: { _id, discountedPrice, name, photo } } = selectedProduct;

                    return { _id, quantity, price: discountedPrice, name, photo }; // Thêm thông tin ảnh và tên sản phẩm
                }
                return null;
